...
ModelPosition::ModelPosition(World* world, Model* parent, const std::string& type) :
    ...
    // private
    velocity(), goal(0, 0, 0, 0), control_mode(CONTROL_VELOCITY), drive_mode(DRIVE_DIFFERENTIAL), localization_mode(
        LOCALIZATION_GPS), integration_error(drand48() * INTEGRATION_ERROR_MAX_X - INTEGRATION_ERROR_MAX_X / 2.0,
                                             drand48() * INTEGRATION_ERROR_MAX_Y - INTEGRATION_ERROR_MAX_Y / 2.0,
                                             drand48() * INTEGRATION_ERROR_MAX_Z - INTEGRATION_ERROR_MAX_Z / 2.0,
                                             drand48() * INTEGRATION_ERROR_MAX_A - INTEGRATION_ERROR_MAX_A / 2.0), wheelbase(
        1.0), acceleration_bounds(), velocity_bounds(),
    //public
...
// install sensible velocity and acceleration bounds
  ...
  for (int i = 0; i < 6; i++)
    this->actuation_noise_parameters[i] = 0;
}
...
// load steering mode
  if (wf->PropertyExists(wf_entity, "drive"))
  {
    ...
    if (mode_str == "diff")
      ...
      PRINT_ERR1(
          "invalid position drive mode specified: \"%s\" - should be one of: \"diff\", \"omni\" or \"car\". Using \"diff\" as default.",
          mode_str.c_str());
...
// choose a localization model
  ...
  wf->ReadTuple(wf_entity, "acceleration_bounds", 0, 8, "llllllaa", &acceleration_bounds[0].min,
                &acceleration_bounds[0].max, &acceleration_bounds[1].min, &acceleration_bounds[1].max,
                &acceleration_bounds[2].min, &acceleration_bounds[2].max, &acceleration_bounds[3].min,
                &acceleration_bounds[3].max);

  wf->ReadTuple(wf_entity, "velocity_bounds", 0, 8, "llllllaa", &velocity_bounds[0].min, &velocity_bounds[0].max,
                &velocity_bounds[1].min, &velocity_bounds[1].max, &velocity_bounds[2].min, &velocity_bounds[2].max,
                &velocity_bounds[3].min, &velocity_bounds[3].max);

  wf->ReadTuple(wf_entity, "velocity_bounds", 0, 6, "llllll", &actuation_noise_parameters[0],
                &actuation_noise_parameters[1], &actuation_noise_parameters[2], &actuation_noise_parameters[3],
                &actuation_noise_parameters[4], &actuation_noise_parameters[5]);
}

double random_normal()
/* normal distribution, centered on 0, std dev 1 */
{
  return sqrt(-2.0 * log(drand48())) * cos(2.0 * M_PI * drand48());
}
  ...
  if (this->subs) // no driving if noone is subscribed
  {
    ...
    switch (control_mode)
    {
      case CONTROL_ACCELERATION:
      {
        ...
        PRINT_DEBUG( "acceleration control mode" );PRINT_DEBUG4( "model %s command(%.2f %.2f %.2f)",
            this->Token(),
            this->goal.x,
            this->goal.y,
            //this->goal.z,
      ...
      {
        PRINT_DEBUG( "velocity control mode" );PRINT_DEBUG4( "model %s command(%.2f %.2f %.2f)",
            this->Token(),
            this->goal.x,
            this->goal.y,
            this->goal.a );

        Pose thrun_noisy_goal = goal;
        float v_dev, omega_dev, phi_dev;
        switch (drive_mode)
        {
          case DRIVE_DIFFERENTIAL:
            // differential-steering model, like a Pioneer
            ...
            v_dev = this->actuation_noise_parameters[0] * vel.x * interval
                + this->actuation_noise_parameters[1] * vel.a * interval;

            omega_dev = this->actuation_noise_parameters[2] * vel.x * interval

            + this->actuation_noise_parameters[3] * vel.a * interval;

            phi_dev = this->actuation_noise_parameters[4] * vel.x * interval
                + this->actuation_noise_parameters[5] * vel.a * interval;

            thrun_noisy_goal.x = goal.x + v_dev * random_normal();
            thrun_noisy_goal.a = goal.a + omega_dev * random_normal();

            if (vel.x < thrun_noisy_goal.x) //accelerating
            {
              vel.x = std::min(thrun_noisy_goal.x, vel.x + acceleration_bounds[0].max * interval);
            }
            else if (vel.x > thrun_noisy_goal.x) //braking
            {
              vel.x = std::max(thrun_noisy_goal.x, vel.x + acceleration_bounds[0].min * interval);
            }

            if (vel.a < thrun_noisy_goal.a) //right
            {
              vel.a = std::min(thrun_noisy_goal.a, vel.a + acceleration_bounds[3].max * interval);
            }
            else if (vel.a > thrun_noisy_goal.a) //left
            {
              vel.a = std::max(thrun_noisy_goal.a, vel.a + acceleration_bounds[3].min * interval);
            }

            if (vel.y < thrun_noisy_goal.y)
            {
              vel.y = std::min(thrun_noisy_goal.y, vel.y + acceleration_bounds[1].max * interval);
            }
            else if (vel.y > thrun_noisy_goal.y)
            {
              vel.y = std::max(thrun_noisy_goal.y, vel.y + acceleration_bounds[1].min * interval);
            }

            break;

          case DRIVE_OMNI:
            // direct steering model, like an omni-directional robot
            vel.x = goal.x;
            vel.y = goal.y;
            vel.a = goal.a;
            break;
    ...
    watts = WATTS + fabs(vel.x) * WATTS_KGMS * mass + fabs(vel.y) * WATTS_KGMS * mass + fabs(vel.a) * WATTS_KGMS * mass;
...
// find the change of pose due to our velocity vector
  const Pose dp(velocity.x * interval, velocity.y * interval, velocity.z * interval, normalize(velocity.a * interval));
