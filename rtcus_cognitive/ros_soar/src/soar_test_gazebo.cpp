...
#define ANGULAR_SPEED_RATIO 1	/* Angular speed tune: above 1 more angular speed. Below 1 
				   less angular speed */
...
#define SOAR_FILE_PATH "/src/sp-look-for-walls-07-metres.soar" /* This constant set the .soar file to be 
...
// Filter the laser readings of each sonar. It takes a laser reading position and returns the minimum of its 
// neighbours (number_readings) values.
float filter_laser_readings (const sensor_msgs::LaserScanConstPtr& msg, int laser_pos, int number_readings) {
	...
	float minimun=6; // Laser scan has 4 metres length area.
	...
	if (valid_readings > 0) {
		...
		// If lasers could not read anything return maximun distance
		return 4.0;
	}
}
/**/
...
void mapErraticMessageToFloat(const sensor_msgs::LaserScanConstPtr& msg){
	// Hokuyo URG-04LX has 240º scanning area, whreas in Gazebo the laser has 270º scanning area.
	sonars_output[0] = filter_laser_readings (msg, 114, LASERS_PER_READING);
	sonars_output[1] = filter_laser_readings (msg, 190, LASERS_PER_READING);
	sonars_output[2] = filter_laser_readings (msg, 266, LASERS_PER_READING);
	sonars_output[3] = filter_laser_readings (msg, 340, LASERS_PER_READING);
	sonars_output[4] = filter_laser_readings (msg, 342, LASERS_PER_READING);
	sonars_output[5] = filter_laser_readings (msg, 417, LASERS_PER_READING);
	sonars_output[6] = filter_laser_readings (msg, 493, LASERS_PER_READING);
	sonars_output[7] = filter_laser_readings (msg, 568, LASERS_PER_READING);
	

	// Monitor laser readings with 'rxconsole' command in ROS:
	/*ROS_INFO("Laser ranges (%d): \n 0: %.2f \n 1:%.2f \n 2:%.2f \n 3:%.2f \n 4:%.2f \n 5:%.2f \n 6:%.2f \n 7:%.2f", (msg->ranges.size()),
			msg->ranges[114], msg->ranges[190], msg->ranges[266] ,msg->ranges[340] ,msg->ranges[342] ,msg->ranges[417] ,msg->ranges[493] ,msg->ranges[568]);
	*/
}
...
int main(int argc, char **argv) {
	...
	ros::Subscriber sub = n.subscribe("sonar", 1, mapErraticMessageToFloat);
